...
import {
  ...
  AlertTriangle, ArrowRight, X, Package, Info, RotateCcw, Trash2,
} from 'lucide-react'
...
import { excluirAnaliseShopee } from '@/actions/excluir-analise-shopee'
...
export function ShopeeAnaliseView({ salvas = [] }: { salvas?: MesSalvo[] }) {
  ...
  async function handleExcluir(ano: number, mes: number, label: string) {
    if (!confirm(`Excluir análise Shopee de ${label}?`)) return
    const key = `${ano}-${mes}`
    setExcluindo(key)
    try {
      const result = await excluirAnaliseShopee(ano, mes)
