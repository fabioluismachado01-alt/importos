...
export function DocumentacaoView({ workspaceId = 'default' }: { workspaceId?: string }) {
  ...
  return (
    <>
      ...
      <style>{`
        @media print {
          /* Invoice/PI/CI/PL — A4 paisagem, margem zero, posição fixa numa página */
          @page { size: A4 landscape; margin: 0; }
          /* Shipping Mark — A4 retrato, margens de 12,7mm (0,5"), fluxo natural multi-página */
          @page sm-page { size: A4 portrait; margin: 12.7mm; }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '4px', background: '#f8fafc', border: '1px solid #e2e8f0', padding: '4px 6px', marginBottom: '6px', fontSize: '8px' }}>
        ...
        {(docType === 'PI' || docType === 'CI') && docInfo.payTerms?.trim() && <div><b>PAYMENT:</b> {docInfo.payTerms.trim()}</div>}
